document.addEventListener("DOMContentLoaded", function () {
    ...
    item.addEventListener("click", () => {
      ...
      if (action === "export") {
        ...
        invoices.forEach((inv, i) => {
          const y = 30 + i * 10;
          doc.text(`#${inv.id} | ${inv.client} | ${inv.date} | ₹${inv.total}`, 15, y);
  ...
  invoiceForm.addEventListener("submit", function (e) {
    ...
    invoices.push({ id: invoiceId, date: today, client, desc, rate, hours, taxRate, total: total.toFixed(2) });
    ...
    pdfBtn.onclick = () => {
      ...
      doc.text(`Hours: ${hours} @ ₹${rate}/hr`, 15, y); y += 8;
    ...
    function render(filtered) {
      ...
      filtered.forEach((inv, index) => {
        ...
        item.innerHTML = `
          ...
          <span style="font-size:13px;">Date: ${inv.date} | Total: ₹${inv.total}</span>
